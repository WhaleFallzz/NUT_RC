...
class BertModelIncr(BertModel):
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, output_all_encoded_layers=True, prev_embedding=None,
                prev_encoded_layers=None, mask_qkv=None, task_idx=None, do_extract=False):
        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)
        if do_extract:
            embedding_output = self.embeddings(
...
class BertForMultiTask(PreTrainedBertModel):
    """MultiTask: Extract + Generation"""
    def __init__(self, config, task_type=None, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
    ...
    def forward(self, input_batch=None, task_type=None):
        # 抽取
        if task_type == 'extract':
            outputs = self.bert(input_batch['input_ids'],
            ...
            # outputs[2:]:()
            ex_outputs = (start_logits, end_logits) + outputs[2:]
            ...
            if start_positions is not None and end_positions is not None:
                ...
                end_loss = loss_fct(end_logits, end_positions)
                total_loss = (start_loss + end_loss) / 2
                ex_outputs = (total_loss,) + outputs
            
            return ex_outputs   # (loss), start_logits, end_logits, (hidden_states), (attentions)

        # 生成
        if task_type == 'generation':
            task_idx = input_batch['task_idx']
            sequence_output, pooled_output = self.bert(input_batch['input_ids'], 
                                    token_type_ids=input_batch['segment_ids'], 
                                    attention_mask=input_batch['attention_mask'], 
                                    task_idx=task_idx,
                                    output_all_encoded_layers=False)
            ...
            masked_lm_labels = input_batch['lm_label_ids']
            masked_pos = input_batch['masked_pos']
            masked_weights = input_batch['masked_weights']
...
class BertForPreTrainingMultiTask(PreTrainedBertModel):
    """refer to BertForPreTraining"""

    def __init__(self, config, num_labels=2, num_rel=0, num_sentlvl_labels=0, no_nsp=False):
        super(BertForPreTrainingMultiTask, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(
            config, self.bert.embeddings.word_embeddings.weight, num_labels=num_labels)
        self.num_sentlvl_labels = num_sentlvl_labels
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                ...
                pair_loss_mask=None, masked_pos_2=None, masked_weights_2=None, masked_labels_2=None,
                num_tokens_a=None, num_tokens_b=None, mask_qkv=None, 
                start_positions=None, end_positions=None, para_query_pos=None, match_score=None):
        ...
        # Extract
        sequence_output_para_query = gather_seq_out_by_pos(sequence_output, para_query_pos)
        
        logits = self.qa_outputs(sequence_output_para_query)
        start_logits, end_logits = logits.split(1, dim=-1)
        start_logits = start_logits.squeeze(-1)
        end_logits = end_logits.squeeze(-1)
        # outputs = (start_logits, end_logits,) + outputs[2:]
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_loss = loss_fct(start_logits, start_positions)
            end_loss = loss_fct(end_logits, end_positions)
            ex_total_loss = (start_loss + end_loss) / 2
        ...
        return masked_lm_loss, ex_total_loss, next_sentence_loss


class BertForEvalMultiTask(PreTrainedBertModel):
    """refer to BertForPreTraining"""

    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0,
                 search_beam_size=1, length_penalty=1.0, eos_id=0, sos_id=0,
                 forbid_duplicate_ngrams=False, forbid_ignore_set=None, ngram_size=3, min_len=0, mode="s2s", pos_shift=False):
        super(BertForEvalMultiTask, self).__init__(config)
        self.bert = BertModelIncr(config)
        # Generation
        self.cls = BertPreTrainingHeads(
            config, self.bert.embeddings.word_embeddings.weight, num_labels=num_labels)
    ...
    def forward(self, input_ids, token_type_ids, position_ids, attention_mask, task_idx=None, mask_qkv=None, 
                task_type=None, ex_token_type_ids=None, ex_position_ids=None, ex_attention_mask=None):
        assert task_type in ['multi_task', 'extract', 'generation']

        # def gather_seq_out_by_pos(seq, pos):
        #     return torch.gather(seq, 1, pos.unsqueeze(2).expand(-1, -1, seq.size(-1)))

        if task_type == 'extract' or task_type == 'multi_task':
            sequence_output, pooled_output = self.bert(
                    input_ids, ex_token_type_ids, ex_position_ids, ex_attention_mask, output_all_encoded_layers=False, mask_qkv=mask_qkv, task_idx=task_idx, do_extract=True)
            # sequence_output_para_query = gather_seq_out_by_pos(sequence_output, para_query_pos)
            ...
            start_logits = start_logits.squeeze(-1)
            end_logits = end_logits.squeeze(-1)
            ex_outputs = (start_logits, end_logits,)

        if task_type == 'generation' or task_type == 'multi_task':
            if self.search_beam_size > 1:
                ge_outputs = self.beam_search(input_ids, token_type_ids, position_ids, attention_mask, task_idx=task_idx, mask_qkv=mask_qkv)
            else:
                input_shape = list(input_ids.size())
                batch_size = input_shape[0]
                input_length = input_shape[1]
                ...
                # return torch.cat(output_ids, dim=1)
                ge_outputs = torch.cat(output_ids, dim=1)

        if task_type == 'multi_task':
            return ex_outputs, ge_outputs
        if task_type == 'extract':
            return ex_outputs
        if task_type == 'generation':
            return ge_outputs
