...
def main():
    ...
    parser.add_argument("--eval_file", type=str, help="Evaluating file")

    parser.add_argument('--subset', type=int, default=0,
                        help="Decode a subset of the input dataset.")
    parser.add_argument("--output_path", type=str, help="output path")
    parser.add_argument("--qid_file", type=str, help="qid file")
    ...
    for model_recover_path in glob.glob(args.model_recover_path.strip()):
        ...
        with open(args.input_file, encoding="utf-8") as fin:
            ...
            if args.subset > 0:
                logger.info("Decding subset: %d", args.subset)
        ...
        if args.output_path:
            ...
            output_path = model_recover_path
        
        if args.split:
            w_file = 'predictions_' + args.split + '.json'
        else:
            w_file = 'predictions_dev.json'

        ge_output_file = os.path.join(output_path, w_file)

        # ge
        with open(args.qid_file, 'r') as f_qid:
            qids = f_qid.readlines()
            predictions = []
            for ans, qid in zip(output_lines, qids):
                predictions.append({
                    'qid': qid.strip(),
                    'Answer': ans,
                })
        with open(ge_output_file, 'w') as writer:
            writer.write(json.dumps(predictions, indent=4) + '\n')

        # Evaluate with the official TweetQA script
        if args.split != 'test':
            results = evaluate_on_tweetqa(gold_file=args.eval_file, 
                                    pred_file=ge_output_file) 
            return results
