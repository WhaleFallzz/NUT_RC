...
from pytorch_pretrained_bert.modeling import BertForPreTrainingMultiTask
...
def main():
    ...
    if args.do_train:
        ...
        bi_uni_pipeline = [seq2seq_loader.Preprocess4MultiTask(args.max_pred, args.mask_prob, list(tokenizer.vocab.keys(
        ...
        # MultiTaskDataset
        train_dataset = seq2seq_loader.MultiTaskDataset(
            fn_src, fn_tgt, args.train_batch_size, data_tokenizer, args.max_seq_length, file_oracle=file_oracle, bi_uni_pipeline=bi_uni_pipeline, bert_tokenizer=tokenizer)
    ...
    if (recover_step is None) and (args.model_recover_path is None):
        ...
        model = BertForPreTrainingMultiTask.from_pretrained(
    ...
    if args.do_train:
        ...
        for i_epoch in trange(start_epoch, int(args.num_train_epochs)+1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
            ...
            for step, batch in enumerate(iter_bar):
                ...
                if args.has_sentence_oracle:
                    ...
                    input_ids, segment_ids, input_mask, mask_qkv, lm_label_ids, masked_pos, masked_weights, is_next, task_idx, start_position, end_position, para_query_pos, match_score = batch
                    oracle_pos, oracle_weights, oracle_labels = None, None, None
                loss_tuple = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next, masked_pos=masked_pos, masked_weights=masked_weights, 
                                    task_idx=task_idx, mask_qkv=mask_qkv,
                                    masked_pos_2=oracle_pos, masked_weights_2=oracle_weights, masked_labels_2=oracle_labels, 
                                    start_positions=start_position, end_positions=end_position, para_query_pos=para_query_pos, match_score=match_score)
                
                masked_lm_loss, ex_loss, next_sentence_loss = loss_tuple
                if n_gpu > 1:    # mean() to average on multi-gpu.
                    # loss = loss.mean()
                    masked_lm_loss = masked_lm_loss.mean()
                    next_sentence_loss = next_sentence_loss.mean()
                    ex_loss = ex_loss.mean()

                # loss直接相加
                loss = masked_lm_loss + next_sentence_loss + ex_loss
                if next_sentence_loss != 0:
        ...
        # Save a finally trained model
        logger.info(
            "** ** * Saving fine-tuned model and optimizer ** ** * ")
        model_to_save = model.module if hasattr(
            model, 'module') else model  # Only save the model it-self
        output_model_file = os.path.join(
            args.output_dir, "pytroch_model.bin")
        torch.save(model_to_save.state_dict(), output_model_file)
        # output_optim_file = os.path.join(
        #     args.output_dir, "optim.bin".format(i_epoch))
        # torch.save(optimizer.state_dict(), output_optim_file)
